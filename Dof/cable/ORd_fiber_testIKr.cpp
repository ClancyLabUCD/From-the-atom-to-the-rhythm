...
const double Drug= 0.0; //OR 1.4nM
    ...
    for (int subjectCase = 1; subjectCase <= 1; subjectCase++)  {
        ...
        while (!done) {
            ...
            if (S->beat <= 4000) {
                ...
                cycle_length = BCL - 200;
            } else if (S->beat == 4002){
                cycle_length = BCL + 4000 ;
...
double Calcu_I_Total(Cell *theCell, double dt, double I_inj){
    ...
    //Inactive
    double Kd_neu_inac = Kd_neu_open/70; //nM ;//70-fold decrease than Kd_neu_open
    
    double Kd_cha_inac = Kd_cha_open/70; ///nM

//O'Hara-Rudy Human model
//**********************************************
//O'Hara T, Virág L, Varró A, Rudy Y. 
//"Simulation of the undiseased human cardiac ventricular action potential: model formulation and experimental validation". 
//PLoS Comput Biol. 2011 May;7(5):e1002061. Epub 2011 May 26.
//**********************************************
//
//Created by Pei-Chi Yang 
//
//Colleen Clancy Lab @ UC davis
//
//June-21-2019
//Ikr Drug model - Mox
// To simulated Figure 5 (A & B) in the paper: A computational pipeline to predict cardiotoxicity:
...
const double Drug = 2.72;  //2.72 (ng/ml)
...
int main () {
	...
	const char *stateFileName = "SingleState1Hz";
	const char *fromSingle = "SingleState1Hz_Dof2.beat2000"; //with drug
    //const char *fromSingle = "SingleState1Hz_Dof0.beat2000"; //without drug (control case)
	double vmax = 0;
    ...
    theCell->C2_Kr = 0;
    theCell->O_Kr = 0;
    theCell->C1_Kr = 1-(theCell->I_Kr + theCell->C3_Kr + theCell->C2_Kr + theCell->O_Kr +theCell->In_drug +theCell->C3n_drug+theCell->C2n_drug+theCell->C1n_drug+theCell->C3c_drug+theCell->C2c_drug+theCell->C1c_drug+theCell->Ic_drug);
    theCell->In_drug = 0;
    theCell->C3n_drug = 0;
    theCell->C2n_drug = 0;
    theCell->On_drug = 0;
    theCell->C1n_drug = 0;
        ...
        sprintf(name, ("vm"+str+"%d.txt").c_str(),drugname);
        output = fopen(name, "w");
        cout << name << endl;
        
        sprintf(name, ("APD"+str+"%d.txt").c_str(),drugname);
        output1=fopen ( name, "w");
        cout << name << endl;

    } else {
        cout << "restarting from Single S-State " << endl;
	...
	while (!done) {
        ...
        if ( V > highestV - 0.3 * ( highestV - lowestV) ) {
            tt2 = S->t;
        }
        
        if ( V > highestV - 0.9 * ( highestV - lowestV) ) {
            tt3 = S->t;
        }
		
		if(  S->beat>0 && ( S->counter % (fold*1) == 0 ) ) {
			  fprintf (output, "%10.3f\t%8.5f\t%8.5f\n",S->tt, S->cellData.v, S->cellData.xr);
		}	
		...
		if(S->t >= cycle_length) {
            ...
            fprintf (output1, "%d\t%8.5f\t%8.5f\n", S->beat, tt2 - tt0, tt3-tt0);
            S->t = 0;
            S->beat++;
...
double Calcu_I_Total(Cell *theCell, double dt, double I_inj){
    ...
    const double pH=7.2;
    const double pKa=7.0005;
    
    const double portion = 1/(1+ pow(10, (pH-pKa)) );
    
    //open
    
    double Kd_neu_open = 159.2; //nM
    
    double Kd_cha_open = 65086; //nM
    
    
    //Inactive
    double Kd_neu_inac = Kd_neu_open/70;//70-fold decrease than Kd_neu_open
    
    double Kd_cha_inac = Kd_cha_open/70;//70-fold decrease than Kd_cha_open
    
    double Dofetilide = Drug / 441.567*1E3; // convert to nM
   

    
    double drug_charged = Dofetilide * portion;
    
    double drug_neutral = Dofetilide * (1-portion);
    
    double Inputs[] = {
       0.001717223376007748,	1.50711828518064
    };
    
    double kOD, kID, rOD, rID, kO_D, kI_D, rO_D, rI_D, aid, ai_d;
    
    
    if (Drug == 0 || drug_charged == 0 || drug_neutral == 0) {
        ...
        // neutral drug
        kO_D = 6.66e-04  * drug_neutral;//nM * drug_neutral;//Inputs[0]
        kI_D = Inputs[0] * drug_neutral; //set different than kOD to test - see test file 5.36e-04* drug_neutral;//
        rO_D = 6.66e-04 * Kd_neu_open; //1.06e-1;//
        rI_D = Inputs[0] * Kd_neu_inac ; //2.57e+2;//
        ai_d = ( bi_d * rI_D * ai * kO_D ) / ( rO_D * bi * kI_D ); //neutral
        
        // charged drug
        kOD = 5.34e-4 * drug_charged;//nM 5.36e-4* drug_charged;//Inputs[2]
        kID = Inputs[1] * drug_charged; //6.06e-4* drug_charged;//
        rOD = 5.34e-4 * Kd_cha_open; //3.47;//
        rID = Inputs[1] * Kd_cha_inac ; //3.33;//
        aid = ( bid * rID * ai * kOD ) / ( rOD * bi * kID ); //charged
...
//    
    sum2=theCell->O_Kr+theCell->C1_Kr+theCell->C2_Kr+theCell->C3_Kr+theCell->I_Kr+ theCell->On_drug + theCell->C1n_drug + theCell->C2n_drug + theCell->C3n_drug + theCell->In_drug + theCell->Oc_drug + theCell->C1c_drug + theCell->C2c_drug + theCell->C3c_drug + theCell->Ic_drug;
//    
